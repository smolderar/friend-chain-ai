...
var blockchainNetworkMap = {
  ...
  [`${Blockchain.Base}:${NetworkId.Main}`]: 80 | 1,
  [`${Blockchain.Base}:${NetworkId.Sepolia}`]: 80 | 2,
  [`${Blockchain.Bnb}:${NetworkId.Main}`]: 96 | 1,
  [`${Blockchain.Bnb}:${NetworkId.Test}`]: 96 | 2
};
...
var SchemaHash = class _SchemaHash {
   ...
   */
  marshalTextBytes() {
    return Hex2.encode(this.bytes);
  }
  marshalText() {
    return Hex2.encodeString(this.bytes);
  }
  /**
  ...
  static newSchemaHashFromHex(s) {
    const schemaEncodedBytes = Hex2.decodeString(s);
...
// src/id.ts
import { poseidon as poseidon2, base58ToBytes, base58FromBytes } from "@iden3/js-crypto";
var Id = class _Id {
  _bytes;
  _checksum;
  ...
  static profileId(id, nonce) {
    const bigIntHash = poseidon2.hash([id.bigInt(), nonce]);
...
// src/claim.ts
import { Hex as Hex3, poseidon as poseidon3 } from "@iden3/js-crypto";
...
var Claim = class _Claim {
  ...
  hIndex() {
    return poseidon3.hash(ElemBytes.elemBytesToInts(this._index));
  }
  ...
  hValue() {
    return poseidon3.hash(ElemBytes.elemBytesToInts(this._value));
  }
  ...
  hex() {
    ...
    return Hex3.encodeString(b);
  }
  fromHex(hex) {
    const b = Hex3.decodeString(hex);
...
// src/did/did.ts
import { sha256 as sha2562 } from "@iden3/js-crypto";
var DID = class _DID {
  method = "";
  id = "";
  idStrings = [];
  ...
  static idFromUnsupportedDID(did) {
    const hash = sha2562(encoder.encode(did.string()));
