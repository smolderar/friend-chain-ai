import { AuthPubSignals } from '../circuits/auth.js';
import { v4 as uuidv4 } from 'uuid';
import { Circuits } from '../circuits/registry.js';
import { proving, Token } from '@iden3/js-jwz';
import { CircuitId, JWSPacker, KMS, PackageManager, PROTOCOL_CONSTANTS, VerificationHandlerFunc, ZKPPacker, NativeProver, FSCircuitStorage, cacheLoader, byteEncoder, verifyExpiresTime, parseAcceptProfile, CircuitLoadMode, getGroupedCircuitIdsWithSubVersions } from '@0xpolygonid/js-sdk';
...
export class Verifier {
    ...
    static async newVerifier(params) {
        if (!params.suite) {
            ...
            const dirname = params.circuitsDir ?? path.join(__dirname, '..', '..', '..', '..', 'circuits');
    ...
    **/
    ...
    async setupAuthZKPPacker(circuitStorage) {
        ...
        const provingParamMap = new Map();
        const stateVerificationFn = async (circuitId, pubSignals) => {
            if (![CircuitId.AuthV2, CircuitId.AuthV3, CircuitId.AuthV3_8_32].includes(circuitId)) {
                throw new Error(`CircuitId is not supported ${circuitId}`);
            }
            const verifier = new AuthPubSignals(pubSignals);
            await verifier.verifyStates(this.stateResolver);
    ...
    async verifyAuthResponse(response, request, opts) {
        ...
        for (const proofRequest of requestScope) {
            ...
            const allCircuitsSubversions = getGroupedCircuitIdsWithSubVersions(proofRequest.circuitId);
