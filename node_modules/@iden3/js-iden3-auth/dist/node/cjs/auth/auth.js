...
const auth_1 = require("../circuits/auth");
...
class Verifier {
    ...
    static async newVerifier(params) {
        if (!params.suite) {
            ...
            const dirname = params.circuitsDir ?? path_1.default.join(__dirname, '..', '..', '..', '..', 'circuits');
            const circuitStorage = new js_sdk_1.FSCircuitStorage({
                dirname
            });
            params.suite = {
                documentLoader,
    ...
    **/
    async setupAuthV2ZKPPacker(circuitStorage) {
        ...
        const authV2Set = await circuitStorage.loadCircuitData(js_sdk_1.CircuitId.AuthV2, {
            mode: js_sdk_1.CircuitLoadMode.Verification
        });
        ...
        const stateVerificationFn = async (circuitId, pubSignals) => {
            ...
            const verifier = new auth_1.AuthPubSignals(pubSignals);
    ...
    async setupAuthZKPPacker(circuitStorage) {
        if (!circuitStorage) {
            throw new Error('circuit storage is not defined');
        }
        const provingParamMap = new Map();
        const stateVerificationFn = async (circuitId, pubSignals) => {
            if (![js_sdk_1.CircuitId.AuthV2, js_sdk_1.CircuitId.AuthV3, js_sdk_1.CircuitId.AuthV3_8_32].includes(circuitId)) {
                throw new Error(`CircuitId is not supported ${circuitId}`);
            }
            const verifier = new auth_1.AuthPubSignals(pubSignals);
            await verifier.verifyStates(this.stateResolver);
        ...
        const verificationFn = new js_sdk_1.VerificationHandlerFunc(stateVerificationFn);
        const instances = [
            js_jwz_1.proving.provingMethodGroth16AuthV2Instance,
            js_jwz_1.proving.provingMethodGroth16AuthV3Instance,
            js_jwz_1.proving.provingMethodGroth16AuthV3_8_32Instance
        ];
        const sets = await Promise.all(instances.map(({ methodAlg }) => circuitStorage.loadCircuitData(methodAlg.circuitId, {
            mode: js_sdk_1.CircuitLoadMode.Verification
        })));
        const verificationParamMap = new Map();
    ...
    async verifyAuthResponse(response, request, opts) {
        ...
        for (const proofRequest of requestScope) {
            ...
            const allCircuitsSubversions = (0, js_sdk_1.getGroupedCircuitIdsWithSubVersions)(proofRequest.circuitId);
            if (!allCircuitsSubversions.includes(proofResp.circuitId)) {
                throw new Error(`proof is not given for requested circuit expected: ${allCircuitsSubversions.join(', ')}, given ${proofResp.circuitId}`);
            }
            const isValid = await this.prover.verify(proofResp, proofResp.circuitId);
            if (!isValid) {
                throw new Error(`Proof with circuit id ${proofResp.circuitId} and request id ${proofResp.id} is not valid`);
            }
            const CircuitVerifier = registry_1.Circuits.getCircuitPubSignals(proofRequest.circuitId);
            if (!CircuitVerifier) {
            ...
            // verify query
            const verifier = new CircuitVerifier(proofResp.pub_signals, this.getCircuitOpts(proofResp.circuitId));
    ...
    getCircuitOpts(circuitId) {
        switch (circuitId) {
            case 'linkedMultiQuery3':
                return { queryCount: 3 };
            case 'linkedMultiQuery5':
                return { queryCount: 5 };
            case js_sdk_1.CircuitId.LinkedMultiQuery10:
            case js_sdk_1.CircuitId.LinkedMultiQueryStable:
