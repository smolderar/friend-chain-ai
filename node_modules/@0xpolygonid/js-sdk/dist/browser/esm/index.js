...
import { BytesHelper as BytesHelper3, checkBigIntInField } from "@iden3/js-iden3-core";
...
var BjjProvider = class {
  ...
  async sign(keyId, data) {
    ...
    const i = BytesHelper3.bytesToInt(data);
  ...
  async verify(message, signatureHex, keyId) {
    ...
    return PublicKey4.newFromCompressed(hexToBytes(publicKey)).verifyPoseidon(
      BytesHelper3.bytesToInt(message),
...
import { BytesHelper as BytesHelper4, checkBigIntInField as checkBigIntInField2 } from "@iden3/js-iden3-core";
...
function fieldToByteArray(field) {
  ...
  return BytesHelper4.intToBytes(bigIntField);
}
function dataFillsSlot(slot, newData) {
  return checkBigIntInField2(BytesHelper4.bytesToInt(Uint8Array.from([...slot, ...newData])));
}
function checkDataInField(data) {
  return checkBigIntInField2(BytesHelper4.bytesToInt(data));
}
...
import { BytesHelper as BytesHelper5 } from "@iden3/js-iden3-core";
...
var toPublicKeyJwk = (keyStr, keyType) => {
  switch (keyType) {
    ...
    case "P-384" /* P384 */: {
      ...
      const xBytes = BytesHelper5.intToNBytes(pubJwk.x, coordinateByteLength).reverse();
      const yBytes = BytesHelper5.intToNBytes(pubJwk.y, coordinateByteLength).reverse();
...
import { BytesHelper as BytesHelper6, DID as DID6 } from "@iden3/js-iden3-core";
    ...
    case "authV3" /* AuthV3 */:
      {
        ...
        const challenge = BytesHelper6.bytesToInt(msgHash.reverse());
...
var processProofResponse = (zkProof) => {
  ...
  const metadata = ethers4.AbiCoder.defaultAbiCoder().encode(["string"], [zkProof.circuitId]);
...
import { buildDIDType as buildDIDType2, BytesHelper as BytesHelper7, DID as DID10, Id as Id11 } from "@iden3/js-iden3-core";
...
function isIssuerGenesis(issuer, state) {
  ...
  const arr = BytesHelper7.hexToBytes(state);
  const stateBigInt = BytesHelper7.bytesToInt(arr);
...
import { DID as DID13, ChainIds, getUnixTimestamp as getUnixTimestamp5, BytesHelper as BytesHelper8 } from "@iden3/js-iden3-core";
var ContractRequestHandler = class extends AbstractMessageHandler {
  ...
  async handleContractInvoke(message, ctx) {
    ...
    const zkpResponses = await processZeroKnowledgeProofRequests(
      ...
      {
        ethSigner,
        challenge: challenge ?? BytesHelper8.bytesToInt(hexToBytes(await ethSigner.getAddress())),
        supportedCircuits: this._supportedCircuits
      }
    );
...
import { BytesHelper as BytesHelper9, chainIDfromDID, DID as DID19, Id as Id13 } from "@iden3/js-iden3-core";
...
var packEthIdentityProof = (did) => {
  return `0x${bytesToHex(BytesHelper9.intToBytes(DID19.idFromDID(did).bigInt()))}`;
};
...
var InputGenerator = class {
  ...
  authPrepareInputs = async ({
    ...
    const id = DID23.idFromDID(genesisDID);
...
var ProofService = class {
  ...
  async generateAuthProof(circuitId, identifier, opts) {
    ...
    const challenge = opts.challenge ? BytesHelper10.intToBytes(opts.challenge).reverse() : new Uint8Array(32);
  ...
  async generateAuthInputsCommon(hash, did, circuitId) {
    const challenge = BytesHelper10.bytesToInt(hash.reverse());
    const inputsCxt = await this._inputsGenerator.generateInputs({
      ...
      preparedCredential: {
        ...
        credentialCoreClaim: new Claim9()
      }
    });
...
var IdentityWallet = class {
  ...
  async createAuthCoreClaim(revNonce, seed) {
    ...
    const authClaim = Claim10.newClaim(
      schemaHash,
  ...
  async sign(message, credential) {
    ...
    const signature = await this._kms.sign(keyKMSId, BytesHelper11.intToBytes(payload));
  ...
  async signChallenge(challenge, credential) {
    const keyKMSId = getKMSIdByAuthCredential(credential);
    const signature = await this._kms.sign(keyKMSId, BytesHelper11.intToBytes(challenge));
    return Signature3.newFromCompressed(signature);
  }
